...
From this lesson, we will enter the mysterious world of regular expressions. We can learn it as a foreign language. Learn its 
grammar, familiar with its vocabulary, practice and practice, Accumulate experience... In the end you will be the master of the 
regular expression.

First we learn a method of Object RegExp: test(). It is used to test whether a string matches the RegExp Object. It returns a 
boolean value: true/false. See an example:
...
Next we learn wildcard "^" and "$". "^" matches the begining of string. In contrast to it, the "$" matches the end of the string. 
Example:
var str="abcabc";
...
We can see whether the regular expression is using the "g" option(global match), "^a" always matches the character of the 
beginning of a string; "c$" always matches the character of the end of the string. Using "^." or ".$"  instead of "^a" or "c$", 
regular expressions are still working. The following example uses our knowledge of learning to achieve "Capitalize the first 
letter of the word":
...
In the example above, we first use split() to divide a sentence into words(string array), and then use map() for each word to 
execute replace(). In the replace () using the regular expression /^./ match the first letter of the word, and then use an arrow 
function to converted the results of match to uppercase. Finally, use join () to merge the words into sentences.
...
Comprehensive use of "." "^" and "$" can accurately determine whether a string is in compliance with the specific requirements. 
See some example:
...
The following example demonstrates how to create a regular expression that matches the beginning and end of a word. It may help 
you to solve the following test.
...
Coding in function findSimilarity. function accept two parameters: str, a sentence contains some words, separated by spaces; 
word, a sample word.

Your task is to keep the words that are similar to the sample word, and to remove the other words.

The similarity is defined as: the same length as the sample; the letter at the beginning and the end of word are same as the 
sample too.
