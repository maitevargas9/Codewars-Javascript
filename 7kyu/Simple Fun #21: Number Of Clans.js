...
Let's call two integers A and B friends if each integer from the array divisors is either a divisor of both A and B or neither 
A nor B. If two integers are friends, they are said to be in the same clan. How many clans are the integers from 1 to k, 
...
function numberOfClans(divisors, k) {
  return new Set(
    [...Array(k).keys()].map((el) =>
      divisors.map((div) => ((el + 1) % div === 0 ? 1 : 0)).join("")
    )
  ).size;
}