...
reverse() has only one purpose: reverse the order of the elements. Usage is arrayObject.reverse() and don't need parameters. 
Example:
var arr1=[1,2,3,4,5]
arr1.reverse()
...
The parameter compareFunction is optional. Specifies a function that defines the sort order. If omitted, the array is sorted 
according to each character's Unicode code point value, according to the string conversion of each element. Example:
var arr=['c','a','b'];
arr.sort();
console.log(arr)   //output: [ 'a', 'b', 'c' }

This is an example of an array of strings, if arr is a number array and the parameter is omitted, sort() will first convert the 
number into a string, and then sort the array. Example:
...
This result may not be what we want. If you want to sort the numbers according to the size of their values, you must use a 
parameter, the parameter is a function. A classic example:
var arr=[1,2,3,11,21,31];
...
Usually this is the result we want. (a,b)=>a-b is arrow function. The formal format is: function(a,b){return a-b}. If you forget 
how to use the arrow function, please refer to:#22: Unlock new skills--Arrow function,spread operator and deconstruction
...
If we need to sort by two or more conditions, we need to spend more energy to write a proper comparison function. The following 
example has two sorting conditions, first according to the classification of odd and even order, and then follow the numbers in 
ascending order.
...
Coding in function sortIt. function accept 1 parameters arr, it's a number array. Your task is to sort the array according to 
the specified conditions, and returns a new array(should not modify the original array).
...
function sortIt(arr) {
  return arr
    ...
    .sort(
      (x, y) =>
        arr.filter((v) => v == x).length - arr.filter((v) => v == y).length ||
        y - x
    );
}
