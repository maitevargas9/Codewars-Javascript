...
As you can see, we only learn about usage of "" in this lesson. This lesson content will be very few? No, it's just the opposite. 
It's more than any other lessons.

Because its use is very wide. It can marks the next character as a special character, a multi character metacharacter, a 
backreference, or an octal escape.

When we learn the lessons before, there may be a question in our minds: how do we match those characters with special meanings? 
Such as:. * + ? | ( ) { } [ ].

There are two ways: one is to put it in the [ ]. Most of the special characters in the [ ] will lose a special meaning, only on 
behalf of its own. See example:

var regex=/[*+?.|()]/g;
console.log( "(a*b+c?)|(.)".match(regex) );    
//output: [ '(', '*', '+', '?', ')', '|', '(', '.', ')' ]

We can see that this regular expression is matched to the special character. But there will still be more special characters that 
we can't match, such as \ ^ or some nonprinting characters. Then we need to use the second way : put a \ in front of them, as the 
escape mark. The following is a list of some escape character:
...
The special characters we mentioned above such as . * + ? | ( ) { } [ ] can use the \ as escape characters too. Let's see a 
classic example, used to determine whether the brackets are symmetrical:
...
Each multi-character metacharacter represents a character set that can be used to simplify our regular expression. 
Here are some examples of previous lessons, let's see how we can simplify them:
...
The 3rd and 4th usage of \ is backreference and octal escape. Their use format all are \num (num is a number with one digit or 
two digits). Depending on the using environment, regular expressions will judge the meaning of it.

If \num is preceded by at least num captured subexpressions, \num is a backreference. Otherwise, \num is an octal escape 
value(digits of num must be the octal digits(0-7)). Let's see an example of backreference:

var str="good wood food foot green glass roof";
var reg=/(.)\1/g
console.log( str.match(reg))  
//output: [ 'oo', 'oo', 'oo', 'oo', 'ee', 'ss', 'oo' ]
The regular expression in the above example matches any two identical consecutive characters. It should be noted that, if you 
want to using a backreference, you must define the corresponding sub expressions by using ( ). Like the example above, using 
only /.\1/g is wrong.

If you define more than one subexpression, \1 represents the first sub expression, \2 represents the second sub expression, and 
so on.
...
You might have guessed the task. Yes, This time your task is to write a regular expressions matching all palindrome substring of 
a string.

The rules:

The string contains letters, numbers, underscores and spaces. space is the separator. the substring means the whole of a 
substring that separated by spaces. For example:
