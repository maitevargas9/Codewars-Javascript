...
The first row in reinforces array is the current battlefield. Whenever some letter is killed by bomb, it's replaced by a letter from next 
string in reinforces array on the same position.
The airstrike always starts from the beginning of the battlefield.
The * means a bomb drop place. The each * bomb kills letter only on the battelfield. The bomb kills letter on the same index on battlefield 
plus the adjacent letters.
The letters on the battlefield are replaced after airstrike is finished.
Return string of letters left on the battlefield after the last airstrike. In case there is no any letter left in reinforces on specific 
position, return _.
...
function alphabetWar(reinforces, airstrikes) {
  ...
  const hits = Array(len).fill(0);

  for (let strike of airstrikes) {
    const toHit = Array(len).fill(false);
  ...
  for (let i = 0; i < len; i++) {
    if (hits[i] >= reinforces.length) {
      result += "_";
    } else {
      result += reinforces[hits[i]][i];
    }
  }
